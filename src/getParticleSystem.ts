...
function getParticleSystem(params) {
  const { camera, parent, rate, mouse } = params;
 ...
 // max point size = 512; => console.log(ctx.getParameter(ctx.ALIASED_POINT_SIZE_RANGE));
  ...
  function _AddParticles(timeElapsed) {
    const n = Math.floor(mouse.velocity.length() * rate);
    for (let i = 0; i < n; i += 1) {
      const life = (Math.random() * 0.75 + 0.25) * maxLife;
      _particles.push({
        position: new THREE.Vector3(
          gaussianRandom(0, 1) * radius,
          gaussianRandom(0, 1) * radius,
          gaussianRandom(0, 1) * radius
        )
          .add(new THREE.Vector3(mouse.plane.x, mouse.plane.y, 0)),
        ...
        velocity: new THREE.Vector3(mouse.velocity.x * 500, mouse.velocity.y * 500, 0),
      });
    }
  }
